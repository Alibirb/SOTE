...
void	GLDebugDrawer::drawSphere(btScalar radius, const btTransform& transform, const btVector3& color)
{
	if( !getEnabled() )
		return;
	...
	btVector3 xaxis = btVector3(1,0,0);
	btVector3 yaxis = btVector3(0,1,0);
	btVector3 zaxis = btVector3(0,0,1);
	// drawArc(center, normal, axis, radiusA, radiusB, minAngle, maxAngle, color, drawSect, stepDegrees=10.0f)
	drawArc(center, transform.getBasis()*zaxis, transform.getBasis()*xaxis, radius, radius, 0, SIMD_2_PI, color, false, 10.0f);	// xy
	drawArc(center, transform.getBasis()*yaxis, transform.getBasis()*xaxis, radius, radius, 0, SIMD_2_PI, color, false, 10.0f);	// xz
	drawArc(center, transform.getBasis()*xaxis, transform.getBasis()*yaxis, radius, radius, 0, SIMD_2_PI, color, false, 10.0f);	// yz
}
