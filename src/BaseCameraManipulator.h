...
public:
	...
	void translate(osg::Vec3 translation);
	 ...
	 *
	 *  Eye parameter is user position, rotation is the rotation to be fixed, and disallowFlipOver, when set on true, avoids pitch rotation component to grow over +/- 90 degrees. If this happens and disallowFlipOver is true, manipulator is rotated by 180 degrees. More precisely, roll rotation component is changed by 180 degrees, making pitch once again between -90..+90 degrees limits.
	 */
	...
	float getHeading() {return _azimuth;}
	float getZenith() {return _zenith;}
...
protected:
	virtual bool handleFrame(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)			{return false;}
	virtual bool handleResize(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa);
	virtual bool handleMousePush(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)	  	{return false;}
	virtual bool handleMouseRelease(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)	{return false;}
	virtual bool handleMouseMove(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)		{return false;}
	virtual bool handleMouseDrag(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)		{return false;}
	virtual bool handleMouseScroll(const GUIEventAdapter& ea, GUIActionAdapter& aa)					{return false;}
