 ...
 * This is largely a copy of osgGA::GUIEventAdapter
 */
class Event
{
public:
	/// Originally taken from OSG
	enum MouseButton {
		LEFT_MOUSE_BUTTON     = 1<<0,
		MIDDLE_MOUSE_BUTTON   = 1<<1,
		RIGHT_MOUSE_BUTTON    = 1<<2,
		FORWARD_MOUSE_BUTTON  = 1<<3,
		BACKWARD_MOUSE_BUTTON = 1<<4
	};

	/// Originally taken from OSG. Names are changed a bit
	enum EventType {
		NONE                = 0,
		MOUSE_PUSH          = 1<<0,
		MOUSE_RELEASE       = 1<<1,
...
protected:
	...
	const osgGA::GUIEventAdapter* _osgEvent;	/// The OSG event adapter

public:
	Event();
	Event(const osgGA::GUIEventAdapter& osgEvent);	/// Creates an Event from an osgGA::GUIEventAdapter
	virtual ~Event();

	void setEventType(EventType eventType) {_eventType = eventType;}
	EventType getEventType() const {return _eventType;}

	void setKey(int key) { _key = (Key) key; }
	int getKey() const { return _key; }

	void setButton(int button) { _button = (MouseButton) button; }
	MouseButton getButton() const { return _button; }

	const osgGA::GUIEventAdapter* getOsgEvent() const {return _osgEvent;}
