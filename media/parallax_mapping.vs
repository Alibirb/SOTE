...
void main()
{
	...
	mat3 TBNMatrix = mat3( gl_ModelViewMatrix[0].xyz, gl_ModelViewMatrix[1].xyz, gl_ModelViewMatrix[2].xyz ) * mat3( a_tangent.xyz, a_binormal.xyz, gl_Normal );

	//Convert light vector into tangent space
	//v_lightVector = gl_LightSource[0].position.xyz - ecPosition;
	v_lightVector = gl_LightSource[1].position.xyz - ecPosition;
