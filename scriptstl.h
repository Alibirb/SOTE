...
// TODO: support more containers, erase, swap and algorithms (eg. sort, reverse)
...
//#define SCRIPT_ASSERT(expr, format, ...) { int r; if ((r = (expr)) < 0){ std::fprintf(stderr, "SCRIPT ERROR (%i) "format, r, __VA_ARGS__); return false; } }	// Original
#define SCRIPT_ASSERT(expr, format, ...) { int r; if ((r = (expr)) < 0){ std::fprintf(stderr, "SCRIPT ERROR (%i) " format, r, __VA_ARGS__); return false; } }	// ALTERED
#endif
...
template <typename T> bool RegisterScriptVector(const char *arraytype, const char *type, asIScriptEngine *engine)
{
	...
	{
		SCRIPT_ASSERT(-1, "Cannot register %s, sizes are different\n", arraytype);
	}
	...
	SCRIPT_ASSERT(engine->RegisterTypedef(dtype = sizetype, sizetype_type),
		"Failed to register typedef for %s\n", dtype);
	...
	SCRIPT_ASSERT(engine->RegisterObjectBehaviour(dtype = arraytype, asBEHAVE_CONSTRUCT, "void f()",
		asFUNCTION(STLConstructor<VT>), asCALL_CDECL_OBJFIRST),
			"Failed to register construct behaviour for %s\n", dtype);
	...
	SCRIPT_ASSERT(engine->RegisterObjectBehaviour(dtype = arraytype, asBEHAVE_CONSTRUCT, (std::string("void f(") + arraytype + " &in)").c_str(),
		asFUNCTION(STLCopyConstructor<VT>), asCALL_CDECL_OBJFIRST),
			"Failed to register copy construct behaviour for %s\n", dtype);
	...
	SCRIPT_ASSERT(engine->RegisterObjectBehaviour(dtype = arraytype, asBEHAVE_DESTRUCT, "void f()",
		asFUNCTION(STLDestructor<VT>), asCALL_CDECL_OBJFIRST),
			"Failed to register destruct behaviour for %s\n", dtype);
	...
	SCRIPT_ASSERT(engine->RegisterObjectMethod(dtype = arraytype, (std::string(arraytype) + " &opAssign(const " + arraytype + " &in)").c_str(),
		asMETHODPR(VT, operator=, (const VT &), VT &), asCALL_THISCALL),
			"Failed to register assignment operator for %s\n", dtype);
	...
	/*SCRIPT_ASSERT(engine->RegisterObjectBehaviour(dtype = arraytype, asBEHAVE_INDEX, (std::string(type) + " &f(int)").c_str(),	// Original
		asMETHODPR(VT, at, (size_t), T &), asCALL_THISCALL),
			"Failed to register [] operator for %s\n", dtype);*/
	SCRIPT_ASSERT(engine->RegisterObjectMethod(dtype = arraytype,  (std::string(type) + " &opIndex(int)").c_str(),		// ALTERED
		asMETHODPR(VT, at, (size_t), T &), asCALL_THISCALL),
			"Failed to register [] operator for %s\n", dtype);
	...
	/*SCRIPT_ASSERT(engine->RegisterObjectMethod(dtype = arraytype, decl = (std::string("void erase(...)") + ).c_str(),
		asMETHODCAST(VT, void (VT::*)(...), erase), asCALL_THISCALL),
			"Failed to register %s for %s\n", decl, dtype);*/
...
template <typename T> bool RegisterScriptList(const char *arraytype, const char *type, asIScriptEngine *engine)
{
	...
	/*SCRIPT_ASSERT(engine->RegisterObjectMethod(dtype = arraytype, decl = (std::string("void erase(...)") + ).c_str(),
		asMETHODCAST(LT, void (LT::*)(...), erase), asCALL_THISCALL),
			"Failed to register %s for %s\n", decl, dtype);*/
