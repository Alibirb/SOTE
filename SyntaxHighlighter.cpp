...
/// Until GCC 4.9, the regex functions in libstdc++ were just stumps that returned false. Since we can't rely on everyone having a proper regex implementation in their standard library, we need to use boost.
...
SyntaxHighlighter::SyntaxHighlighter() : _needsParsing(true)
{
	...
	_styles["multiLineComment"].textColor = green;		/// This is only different for testing purposes. Once the kinks are worked out, we'll just use one color for both single-line and multi-line comments.
